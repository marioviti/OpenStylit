...
double style_ssd( py::array_t<double> A, py::array_t<double> B,
                  int a_i, int a_j,
                  int b_i, int b_j,
                  int w) {
  ...
  double sq_style = 0.0, sq_LPE = 0.0;
  for ( size_t offx=0; offx<w; offx++ )
    for ( size_t offy=0; offy<w; offy++ ) {
      for ( size_t c=0; c<3; c++ ){
        sq_style += std::pow(pA(a_i+offx,a_j+offy,c) - pB(b_i+offx,b_j+offy,c),2);
      }
      for ( size_t c=3; c<18; c++ ){
        sq_LPE += std::pow(pA(a_i+offx,a_j+offy,c) - pB(b_i+offx,b_j+offy,c),2);
      }
    }
  return mu1*sq_style+(mu2*sq_LPE);
}
...
void initialization( py::array_t<double> A,
  ...
  for (size_t i=0; i<nnf_h; i++)
    for (size_t j=0; j<nnf_w; j++) {
      ...
      // ssd
      pnnf(i,j,2) = style_ssd(A,B,i,j,nnf_i,nnf_j,patch_size);
    }
}
...
void propagation( int i, int j,
                  ...
                  py::array_t<double> nnf, py::array_t<double> mask_B,
                  int patch_size, int prop_step) {

    auto pmask_B = mask_B.mutable_unchecked<2>();
    ...
    if (pmask_B(min_i,min_j)>0.0) {
      ...
      if (update) {
        ...
        pnnf(i,j,2) = style_ssd(A,B,a_i,a_j,b_i,b_j,patch_size);
      }
    }
}
...
void random_search(int i, int j,
  ...
  while(w > 1) {
    ...
    if (pmask_B(u_i_t,u_j_t)>0.0) {
      if (is_clamped(B_h,B_w, u_i_t,u_i_t+patch_size, u_j_t,u_j_t+patch_size)) {
        new_distance = style_ssd(A,B,i,j,u_i_t,u_j_t,patch_size);
...
void iteration(py::array_t<double> A, py::array_t<double> B,
               py::array_t<double> nnf,
               py::array_t<double> mask_A,
               py::array_t<double> mask_B,
               int patch_size, bool even, int step ) {

    auto pmask_A = mask_A.mutable_unchecked<2>();
    auto bufnnf = nnf.request();
    int nnf_h = bufnnf.shape[0], nnf_w = bufnnf.shape[1];
    if (even){
      ...
      for (int i = inc_left_h; i < exc_right_h; i+=1) {
        for (int j = inc_left_w; j < exc_right_w; j+=1) {
          if (pmask_A(i,j)>0) {
            propagation(i,j,A,B,nnf,mask_B,patch_size,-step);
            random_search(i,j,A,B,nnf,mask_B,patch_size);
          }
        }
      }
    }
                                    ...
                                    int patch_size )
{
    ...
    auto recbuf = reconsturcted.request();
    std::cout << "reconstruction shape:" << '\n';
    std::cout << recbuf.shape[0] << ", " << recbuf.shape[1] <<  ", " << recbuf.shape[2] << '\n';
...
py::array_t<double> reversed_nnf_approx( py::array_t<double> A,
                                py::array_t<double> B,
                                py::array_t<double> nnf,
                                py::array_t<double> mask_A,
                                py::array_t<double> mask_B,
  ...
  for (int i=0; i<iterations; i++) {
    ...
    iteration(A,B,nnf,mask_A,mask_B,patch_size,(i+1)%2==0,1);
  }
  return nnf;
}

PYBIND11_MODULE(patchmatch_reversed, m) {
    m.def("reversed_nnf_approx", &reversed_nnf_approx, "A function for reversed_nnf_approx" );
    m.def("reconstruction", &reconstruction, "A function for reconstruction" );
}
/*
c++ -O3 -Wall -shared -std=c++11 -fPIC `python3 -m pybind11 --includes` patchmatch_reversed.cpp -o patchmatch_reversed`python3-config --extension-suffix`
*/
