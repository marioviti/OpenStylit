...
@jit
def nnf_approx(A,B,nnf=None,distance_function=ssd,patch_size=5,iterations=5,reconstruct=False):
...
@jit
def prop_and_search(A,B,nnf,distance_function,patch_size,even,step,i,j):
    ...
    if even:
        ...
        for i in range( inc_left_h, exc_right_h, d ):
            for j in range( inc_left_w, exc_right_w, d ):
                prop_and_search(A,B,nnf,distance_function,patch_size,even,step,i,j)
